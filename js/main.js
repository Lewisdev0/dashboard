...
$(document).ready(() => {
    ...
    $("#delLast").click(() => {
        if (client.user.lastMessage === null) {
            $("#delLast").html(`<i class="fas fa-times"></i> Son Mesajı silinemiyor! [Hata]`)
            setTimeout(function () {
                $("#delLast").html(`<i class="far fa-calendar-times"></i> Son Mesajı silindi`);
            }, 2000);
            return;
        } else {
            try {
    ...
    $('#leaveGuild').click(() => {
        if ($("#guilds").val() !== 'DM') {
            if (window.confirm("Sunucudan ayrılmaktasınız?")) {
    ...
    $('#btnrRefreshChat').click(() => {
        $("#btnrRefreshChat").html(`<i class="fas fa-sync fa-spin"></i> Sohbet yenileniyor.`);
        updateChannel(client);
        setTimeout(function () {
            $("#btnrRefreshChat").html(`<i class="fas fa-sync-alt"></i> Sohbet yenilendi.`);
        }, 2000);
...
function sendMessage(client) {
    if ($("#toSend").val() === '') {
        $("#send").html(`<i class="fas fa-times"></i> Mesaj gönderilemedi [HATA]`)
        setTimeout(function () {
            $("#send").html(`<i class="fas fa-share-square"></i> Tekrar dene`);
        }, 2000);
        return alert('Mesaj göndermem için bir şeyler yaz.');
    } else {
        if ($("#guilds").val() === 'DM') {
            var user = client.users.find((user) => user.id === $('#channels').val());
